// public/clonefy.js
// Lightweight embed-script voor Clonefy Player

(function () {
  // 1. CSS één keer injecteren
  const PLAYER_STYLE_ID = "clonefy-player-styles-v1";
  ...
  // 2. Je bestaande configuratie (later koppelen we dit aan de API)
  const videoSources = {
    NL: "https://clonefy-video.b-cdn.net/VIDEO%203/VID%203%20-%20NL_2.mp4",
    EN: "https://clonefy-video.b-cdn.net/VIDEO%203/VID%203%20-%20EN_1.mp4",
    AR: "https://clonefy-video.b-cdn.net/VIDEO%203/VID%203%20-%20AR_1.mp4"
  };
  ...
  // 3. HTML-template van de player (zonder <html>/<body>)
  ...
  // 4. Player initialiseren in een container
  function initClonefyPlayer(container) {
    injectStyles();

    if (!container) return;

    container.classList.add("video-player-container", "vp-host");
    ...
    function buildLanguageDropdown() {
      ...
      Object.keys(videoSources).forEach((code) => {
        if (!languageData[code]) return;
        const countryCode = languageData[code].countryCode;
        const flagUrl = "https://flagcdn.com/w40/" + countryCode + ".png";
        const li = document.createElement("li");
        if (code === currentLang) li.classList.add("active-lang");
        else li.onclick = () => switchLanguage(code);
        li.innerHTML =
          '<span class="flag"><img src="' +
          flagUrl +
          '"></span><span>' +
          languageData[code].name +
          "</span>";
    ...
    video.onloadedmetadata = () => {
      if (isFinite(video.duration) && video.duration > 0) {
        totalTimeEl.textContent = fmt(video.duration);
      }
    };
  ...
  // 5. Bootstrap: zoek het element met data-embed-id
  function bootstrap() {
    const container = document.querySelector("[data-embed-id]");
    if (!container) return;
    // Hier kun je later embedId koppelen aan API:
    // const embedId = container.getAttribute("data-embed-id");
    initClonefyPlayer(container);
  }
  ...
  // Optioneel: globale toegang
  window.ClonefyPlayer = {
    init: initClonefyPlayer
  };
})();
